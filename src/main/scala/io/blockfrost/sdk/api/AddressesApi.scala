...
trait AddressesApi[F[_], P] extends SttpSupport {
  ...
  def getSpecificAddress(address: String)(implicit config: Config): F[ApiResponse[Address]]

  def getAddressDetails(address: String)(implicit config: Config): F[ApiResponse[AddressDetail]]

  def getAddressUtxos(address: String, pageRequest: SortedPageRequest = SortedPageRequest())(implicit config: Config): F[ApiResponse[Seq[Utxo]]]

  def getAddressTransactions(address: String, from: Option[String], to: Option[String], pageRequest: SortedPageRequest = SortedPageRequest())(implicit config: Config): F[ApiResponse[Seq[Transaction]]]
}

trait AddressesApiImpl[F[_], P] extends AddressesApi[F, P] {
  this: ApiClient[F, P] =>

  override def getSpecificAddress(address: String)(implicit config: Config): F[ApiResponse[Address]] =
    get(uri"$host/addresses/$address")

  override def getAddressDetails(address: String)(implicit config: Config): F[ApiResponse[AddressDetail]] =
    get(uri"$host/addresses/$address/total")

  override def getAddressUtxos(address: String, pageRequest: SortedPageRequest = SortedPageRequest())(implicit config: Config): F[ApiResponse[Seq[Utxo]]] =
    get(uri"$host/addresses/$address/utxos", Some(pageRequest))

  override def getAddressTransactions(address: String, from: Option[String], to: Option[String], pageRequest: SortedPageRequest = SortedPageRequest())(implicit config: Config): F[ApiResponse[Seq[Transaction]]] = {
    get(
